...
from load_data import load_taskonmy_data
import numpy as np
...
class Save_best(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if colo_acc>self.acc_colo and cate_acc>self.acc_cate:
            save_model(decoder,filepath='./taskonmy_model/best_model_{}.h5'.format(type))
            self.acc_colo=colo_acc
            self.acc_cate=cate_acc
        save_model(decoder,filepath='./taskonmy_model/latest_model_{}.h5'.format(type))
...
test_model_=True
type='autoencoder'
model_path='./taskonmy_model/best_model_{}.h5'.format(type)
batch_size = 32
Epochs = 100

Z_train, Z_test, y_train, y_test, colors, classes = load_taskonmy_data('./dataset_taskonmy',type=type,pp=0.2)
...
if test_model_:
    ...
    plt.tight_layout()
    plt.savefig("./taskonmy_model/output_losses_{}.png".format(type))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./taskonmy_model/output_accs_{}.png".format(type))
    plt.close()
